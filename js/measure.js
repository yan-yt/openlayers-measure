$(function () {  
    ...
    var pointerMoveHandler = function (evt) {  
        ...
        if(output){
            if(type === "LineString"){
                helpTooltipElement.innerHTML = '<span>'+helpMsg+'</span></br><span>总长：<span class="measureInfo">'+output+'</span></span>';
            }else{
                helpTooltipElement.innerHTML = '<span>'+helpMsg+'</span></br><span>总面积：<span class="measureInfo">'+output+'</span></span>';
            }
        }else{
    ...
    function drawPloy(type) {  
        ...
        draw.on('drawstart', function (evt) {   
            ...
            listener = sketch.getGeometry().on('change', function (evt) {  
                ...
                //设置测量提示框的内标签为最终输出结果  
                measureTooltipElement.innerHTML = '<div style = "position: absolute; font-size: 12px; font-family: MicrosoftYaHei; white-space: nowarp; height: 17px; background-color: #fff; color: #666; border: 1px solid #d4d4d4">' +
        ...
        draw.on('drawend', function (evt) {
            ...
            if(type === 'LineString'){
                ...
                sourceArr.push(featurePoint)
                sourceTem.push(featurePoint);
                measureTooltipElement.innerHTML = '<div style = "position: absolute; white-space: nowrap;font-size: 13px;font-family: MicrosoftYaHei;color: #666666 ">' +
                '<span style="padding: 4px 4px;border: 1px solid #4a90e2;background-color:#fff">'+'总长'+':' +
                '<strong style = "color : #4a90e2">' + output + '</strong></span>' +
                '<span id = "measureDis' + drawCount + '" class="supermapol-icons-clear" style = "padding:6px 4px;width:22px;height22px;background-color: #FFFFFF;color:#4A90E2;line-height:22px;text-align:center;cursor: pointer;border: 1px solid #4A90E2;position:relative;top:0px;left:2px;">X' +
                '</span></div>'   
                ...
                sourceTem.push(featurePoint);
                measureTooltipElement.innerHTML = '<div style = "position: absolute; white-space: nowrap;font-size: 13px;font-family: MicrosoftYaHei;color: #666666 ">' +
                '<span style="padding: 4px 4px;border: 1px solid #4a90e2;background-color:#fff">'+'总面积'+':' +
                '<strong style = "color : #4a90e2">' + output + '</strong></span>' +
                '<span id = "measureArea' + drawCount + '" class="supermapol-icons-clear" style = "padding:6px 4px;width:22px;height22px;background-color: #FFFFFF;color:#4A90E2;line-height:22px;text-align:center;cursor: pointer;border: 1px solid #4A90E2;position:relative;top:0px;left:2px;">X' +
                '</span></div>'
    ...
    function createMeasureTooltip() {  
        ...
        //创建一个测量提示的覆盖标注  
        measureTooltip = new ol.Overlay({  //覆盖物
